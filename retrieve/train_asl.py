"""Train SubgraphRAG's retriever as a 0-1 triple classifier with ASL.

This script mirrors ``retrieve/ref/train_selector_asl.py`` while replacing the
standalone selector with the original SubgraphRAG ``Retriever``.  Every triple
in each question graph receives one binary-classification logit.  Validation
reports both the fixed-threshold metrics used for 0-1 classification
(``logit > 0``) and the same top-k recall metrics used by the retriever scripts.
"""
...
def build_swanlab_config(args, retriever_config, emb_size):
    ...
    config["retriever/query_emb_size"] = emb_size
    return config


def build_retriever_config(args):
    """Build a retriever-compatible training config entirely from CLI args."""
    return {
        "env": {
...
def pairwise_ranking_loss(retriever_logits, target_labels):
    """Rank positive triples above negative triples."""
    pos_logits = retriever_logits[target_labels > 0]
    neg_logits = retriever_logits[target_labels <= 0]
    if pos_logits.numel() == 0 or neg_logits.numel() == 0:
        return retriever_logits.new_zeros(())
    return -F.logsigmoid(pos_logits[:, None] - neg_logits[None, :]).mean()


@torch.no_grad()
def eval_epoch(args, config, device, data_loader, model, k_list=None, desc="val"):
    model.eval()
    ...
    for sample in tqdm(data_loader, desc=desc):
        ...
        selected_mask = pred_triple_logits > 0
        ...
        sorted_triple_ids_pred = torch.argsort(pred_triple_logits, descending=True)
    ...
    # Keep selector-compatible aliases so dashboards can compare the two ASL flows.
...
def train_epoch(args, device, train_loader, model, optimizer):
    model.train()
    ...
    for sample in tqdm(train_loader, desc="train"):
        ...
        rank_loss = pairwise_ranking_loss(pred_triple_logits, target_labels)
        loss = asl_loss
...
def main(args):
    ...
    test_set = RetrieverDataset(config=config, split=args.test_split, skip_no_path=args.skip_no_path) if args.test_split else None
    ...
    model = Retriever(emb_size=emb_size, **config["retriever"]).to(device)
    optimizer = Adam(model.parameters(), **config["optimizer"])

    ts = time.strftime("%b%d-%H:%M:%S", time.gmtime())
    output_dir = Path(args.output_dir or f"retriever_asl_{args.dataset}_{ts}")
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "args.json").write_text(json.dumps(vars(args), indent=2), encoding="utf-8")

    config_df = pd.json_normalize(build_swanlab_config(args, config, emb_size), sep="/")
    swanlab.init(
        project=args.swanlab_project or f"retriever-asl-{args.dataset}",
    ...
    for epoch in range(args.num_epochs):
        ...
        val_eval_dict = eval_epoch(args, config, device, val_loader, model, args.val_k_list, desc="val")
        if args.best_metric not in val_eval_dict:
            print(
                f"Warning: best_metric '{args.best_metric}' not found in validation metrics; falling back to 'threshold_f1'.",
                file=sys.stderr,
            )
        ...
        if test_loader is not None:
            test_eval_dict = eval_epoch(args, config, device, test_loader, model, args.test_k_list, desc="test")
        ...
        train_log_dict = train_epoch(args, device, train_loader, model, optimizer)
    ...
    if best_state_dict is None:
        torch.save(last_state if last_state is not None else {
            "config": config,
            "model_state_dict": model.state_dict(),
            "optimizer_state_dict": optimizer.state_dict(),
            "args": vars(args),
...
def parse_args():
    parser = argparse.ArgumentParser(description="ASL training for SubgraphRAG retriever 0-1 triple classification")
